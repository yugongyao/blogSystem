...
//取得js文件->错误处理->合并js文件->压缩js->放到指定位置
...
//取得json文件->错误处理->放到指定位置
gulp.task('json',function(){
    //取得json下所有为.json的文件(**/的意思是包含所有子文件夹)
    gulp.src('app/static/json/**/*.json')
    //错误管理模块(有错误时会自动输出提示到终端上)
    .pipe(plumber())
    //将合并压缩后的文件输出到dist/static/json下(如没有dist目录则自动生成dist)
...
//取得scss文件->错误处理->sass转css->合并css->压缩css->放到指定位置
...
gulp.task('watch', function(){
    ...
    gulp.watch('app/static/json/**/*.json',['json']);
...
gulp.task('redist', function(){
    ...
    runSequence(['html', 'sass','json','php','js','images'],'watch')
})
